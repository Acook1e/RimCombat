/*
 * For modders: Copy this file into your own project if you wish to use this
 * API.
 */
#pragma once
...
// JavaScript console message severity level for use with
...
typedef void (*ConsoleMessageCallback)(PrismaView view,
...
public:
  // Create view.
  virtual PrismaView
  CreateView(const char* htmlPath,
  ...
  virtual void InteropCall(PrismaView view, const char* functionName,
  ...
  // Set focus on view.
  virtual bool Focus(PrismaView view, bool pauseGame = false,
  ...
  virtual void SetScrollingPixelSize(PrismaView view,
                                     int pixelSize) noexcept = 0;
  ...
  virtual void SetInspectorVisibility(PrismaView view,
  ...
  virtual void SetInspectorBounds(PrismaView view, float topLeftX,
                                  float topLeftY, unsigned int width,
...
public:
  ...
  virtual void
  RegisterConsoleCallback(PrismaView view,
                          ConsoleMessageCallback callback) noexcept = 0;
};

// Maps interface types to InterfaceVersion enum values.
// compile-time constraint -- only request interface versions that actually
// exist.
template <typename T>
...
/// Recommended: Send your request during or after
/// SKSEMessagingInterface::kMessage_PostLoad to make sure the dll has already
/// been loaded
[[nodiscard]] inline void*
RequestPluginAPI(InterfaceVersion a_interfaceVersion = InterfaceVersion::V1)
{
  ...
  auto requestAPIFunction = reinterpret_cast<RequestPluginAPIFunc>(
...
/// Returns nullptr if the loaded PrismaUI DLL does not support the requested
/// version.
///
/// Usage:
///   auto* m_prismaUI   =
///   PRISMA_UI_API::RequestPluginAPI<PRISMA_UI_API::IVPrismaUI1>(); auto*
///   m_prismaUIv2 =
