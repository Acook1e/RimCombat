...
void SetSection(std::uint32_t hash)
{
  ...
  if (!map.label.empty())
    ...
    SKSEMenuFramework::SetSection(
        Localization::GetLocalization("Unknown"_h).label.data());
}
void AddSectionItem(std::uint32_t hash,
                    SKSEMenuFramework::Model::RenderFunction func)
{
  const Localization::Entry& map = Localization::GetLocalization(hash);
  if (!map.label.empty())
...
void Text(std::uint32_t hash, auto&&... args)
{
  ...
  if (!map.label.empty())
    ImGuiMCP::Text(
        std::vformat(map.label, std::make_format_args(args...)).data());
}
void Checkbox(std::uint32_t hash, bool* v,
...
void DragInt(std::uint32_t hash, int* v, float v_speed = 1.0f, int v_min = 0,
             int v_max = 0, const char* format = "%d",
             std::function<void()> onChange = nullptr)
{
  const Localization::Entry& map   = Localization::GetLocalization(hash);
  ImGuiMCP::ImGuiSliderFlags flags = ImGuiMCP::ImGuiSliderFlags_None;
  if (ImGuiMCP::DragInt(map.label.data(), v, v_speed, v_min, v_max, format,
                        flags))
    if (onChange)
      onChange();
  if (ImGuiMCP::IsItemHovered() && !map.desc.empty())
...
void DragFloat(std::uint32_t hash, float* v, float v_speed = 1.0f,
               ...
               std::function<void()> onChange = nullptr)
{
  const Localization::Entry& map   = Localization::GetLocalization(hash);
  ImGuiMCP::ImGuiSliderFlags flags = ImGuiMCP::ImGuiSliderFlags_None;
  if (ImGuiMCP::DragFloat(map.label.data(), v, v_speed, v_min, v_max, format,
                          flags))
    if (onChange)
      onChange();
  if (ImGuiMCP::IsItemHovered() && !map.desc.empty())
...
template <typename T>
void Combo(std::uint32_t hash,
           std::vector<std::reference_wrapper<Localization::Entry>> comboIt,
           T* current_item, std::function<void()> onChange = nullptr)
{
  const Localization::Entry& map = Localization::GetLocalization(hash);
  if (ImGuiMCP::BeginCombo(map.label.data(),
                           comboIt[*current_item].label.data())) {
