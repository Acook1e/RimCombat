...
public:
  ...
  /// If granted, you may manipulate the current target and soft target in whatever ways you wish
  /// for the duration of your control.
  /// </summary>
  ...
  /// If granted, you may provide the functions that will be used to get the special bars' fill
  /// percent.
  /// </summary>
  /// <param name="a_myPluginHandle">Your assigned plugin handle</param>
  /// <returns>OK, AlreadyGiven, AlreadyTaken</returns>
  ...
  /// <summary>
  /// Tries to set the current target to the given actor handle. Will only do so if granted target
  /// control.
  /// </summary>
  /// <param name="a_myPluginHandle">Your assigned plugin handle</param>
  ...
  /// <summary>
  /// Tries to set the current soft target to the given actor handle. Will only do so if granted
  /// target control.
  /// </summary>
  /// <param name="a_myPluginHandle">Your assigned plugin handle</param>
  /// <returns>OK, NotOwner</returns>
  ...
  /// <summary>
  /// Tries to send a visual flash event related to the given actor value on a widget related to the
  /// given actor handle (similar to vanilla stamina bar flashing when trying to sprint while it's
  /// empty). Will only succeed if such a target exists and is supported by the widget.
  /// </summary>
  /// <param name="a_actorHandle">Actor handle</param>
  ...
  /// <summary>
  /// Tries to send a visual flash event on a special bar related to the given actor handle. Will
  /// only do so if granted control.
  /// </summary>
  /// <param name="a_myPluginHandle">Your assigned plugin handle</param>
  /// <param name="a_actorHandle">Actor handle</param>
  /// <param name="a_bLong">Play longer flash animation</param>
  ...
  /// </summary>
  /// <param name="a_myPluginHandle">Your assigned plugin handle</param>
  /// <param name="a_getCurrentSpecialResource">Function that will return current special resource
  /// value</param> <param name="a_getMaxSpecialResource">Function that will return max special
  /// resource value</param> <param name="a_bSpecialMode">Whether the max value is default and 0 is
  /// the flash threshold (true), or the other way around (false)</param> <param
  /// name="a_bDisplaySpecialForPlayer">Whether the special bar should be displayed for the player
  /// as well</param> <returns>OK, NotOwner</returns>
  virtual APIResult RegisterSpecialResourceFunctions(SKSE::PluginHandle a_myPluginHandle, SpecialResourceCallback&& a_getCurrentSpecialResource,
                                                     SpecialResourceCallback&& a_getMaxSpecialResource, bool a_bSpecialMode,
  ...
  /// </summary>
  /// <param name="a_myPluginHandle">Your assigned plugin handle</param>
  /// <param name="a_filePath">File path to the .swf file, relative to the Data/Interface
  /// folder</param> <param name="a_successCallback">Function that will be called back when done.
  /// Check the result before proceeding.</param>
  ...
  /// <summary>
  /// Adds a custom widget. Will succeed only if prepared correctly by calling the previous two
  /// functions.
  /// </summary>
  /// <param name="a_myPluginHandle">Your assigned plugin handle</param>
  /// <param name="a_widgetType">A pluginwide unique ID of the widget type.</param>
  ...
  /// <param name="a_widget">The shared pointer to the widget (based on the WidgetBase class)
  /// created in your plugin</param>
  virtual void AddWidget(SKSE::PluginHandle a_myPluginHandle, uint32_t a_widgetType, uint32_t a_widgetID, std::string_view a_symbolIdentifier,
  ...
  /// <param name="a_widgetType">A pluginwide unique ID of the widget type.</param>
  /// <param name="a_widgetID">An unique ID of the widget.</param>
  /// <param name="a_removalMode">Indicates whether the widget should be removed instantly. Modes
  /// other than Immediate have to be handled inside the widget</param>
...
public:
  ...
  virtual void DrawLine(const RE::NiPoint3& a_start, const RE::NiPoint3& a_end, float a_duration = 0.f, uint32_t a_color = 0xFF0000FF,
  ...
  virtual void DrawArrow(const RE::NiPoint3& a_start, const RE::NiPoint3& a_end, float a_size = 10.f, float a_duration = 0.f,
                         uint32_t a_color = 0xFF0000FF, float a_thickness = 1.f) noexcept                                              = 0;
  virtual void DrawBox(const RE::NiPoint3& a_center, const RE::NiPoint3& a_extent, const RE::NiQuaternion& a_rotation, float a_duration = 0.f,
                       uint32_t a_color = 0xFF0000FF, float a_thickness = 1.f) noexcept                                                = 0;
  virtual void DrawCircle(const RE::NiPoint3& a_center, const RE::NiPoint3& a_x, const RE::NiPoint3& a_y, float a_radius, uint32_t a_segments,
                          float a_duration = 0.f, uint32_t a_color = 0xFF0000FF, float a_thickness = 1.f) noexcept                     = 0;
  virtual void DrawHalfCircle(const RE::NiPoint3& a_center, const RE::NiPoint3& a_x, const RE::NiPoint3& a_y, float a_radius, uint32_t a_segments,
                              float a_duration = 0.f, uint32_t a_color = 0xFF0000FF, float a_thickness = 1.f) noexcept                 = 0;
  virtual void DrawSphere(const RE::NiPoint3& a_origin, float a_radius, uint32_t a_segments = 16, float a_duration = 0.f,
                          uint32_t a_color = 0xFF0000FF, float a_thickness = 1.f) noexcept                                             = 0;
  virtual void DrawCylinder(const RE::NiPoint3& a_start, const RE::NiPoint3& a_end, float a_radius, uint32_t a_segments, float a_duration = 0.f,
                            uint32_t a_color = 0xFF0000FF, float a_thickness = 1.f) noexcept                                           = 0;
  virtual void DrawCone(const RE::NiPoint3& a_origin, const RE::NiPoint3& a_direction, float a_length, float a_angleWidth, float a_angleHeight,
                        uint32_t a_segments, float a_duration = 0.f, uint32_t a_color = 0xFF0000FF, float a_thickness = 1.f) noexcept  = 0;
  virtual void DrawCapsule(const RE::NiPoint3& a_origin, float a_halfHeight, float a_radius, const RE::NiQuaternion& a_rotation,
                           float a_duration = 0.f, uint32_t a_color = 0xFF0000FF, float a_thickness = 1.f) noexcept                    = 0;
...
public:
  // Debug drawing API functions
  virtual void DrawCapsule(const RE::NiPoint3& a_vertexA, const RE::NiPoint3& a_vertexB, float a_radius, float a_duration = 0.f,
                           uint32_t a_color = 0xFF0000FF, float a_thickness = 1.f) noexcept = 0;
};
...
/// Recommended: Send your request during or after SKSEMessagingInterface::kMessage_PostLoad to make
/// sure the dll has already been loaded
/// </summary>
