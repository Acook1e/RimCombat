...
public:
  static void install()
  {
    ...
    std::uintptr_t hook  = REL::VariantID(48139, 49170, 0).address() +
                           REL::VariantOffset(0x4D7, 0x435, 0x0).offset();
    _PerformAttackAction = trampoline.write_call<5>(hook, PerformAttackAction);
    logger::info("Hooks: OnAttackAction installed.");
  }

private:
  static bool PerformAttackAction(RE::TESActionData* actionData);
  static inline REL::Relocation<decltype(PerformAttackAction)>
...
public:
  static void install()
  {
    REL::Relocation<std::uintptr_t> atkbckVtbl{
...
private:
  static void ProcessButton(RE::AttackBlockHandler* this,
